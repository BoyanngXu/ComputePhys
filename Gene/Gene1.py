...
def fitness(n_individuals):
    ...
    for j in range(len(n_individuals)):
        ...
        for i in range(len(n_individuals)):
            ...
            r = np.linalg.norm(np.array([x,y,z])-np.array([x_0, y_0, z_0]))
            # if r != 0:
            #     energy += 1/r
            # else: continue
            fitnesses += r
...
# 定义遗传算法函数
def genetic_algorithm(population_size=20, num_generations=1000, crossover_rate=0.7, mutation_rate=0.01, begin_time = time.time()):
